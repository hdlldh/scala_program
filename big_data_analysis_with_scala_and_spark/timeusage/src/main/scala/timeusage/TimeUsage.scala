...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val f1 = List("t01", "t03", "t11", "t1801", "t1803")
    val groups = columnNames.groupBy{
      name =>
        if(name.startsWith("t01") ||
          ...
          "primary"
        else if(name.startsWith("t05") ||
          name.startsWith("t1805"))
          "work"
        else if(name.startsWith("t02") ||
          name.startsWith("t04") ||
          name.startsWith("t06") ||
          name.startsWith("t07") ||
          name.startsWith("t08") ||
    ...
    (groups("primary").map(x => new Column(x)),
      groups("work").map(x => new Column(x)),
      groups("other").map(x => new Column(x))
    )
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy($"working", $"sex", $"age").agg(round(avg($"primaryNeeds"), 1).as("primaryNeed"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(x: Double) = math.round(x*10).toDouble / 10
