...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAcc(chars: List[Char], acc: Map[Char, Int]): Map[Char, Int] = chars match {
      case Nil => acc
      case ch :: chs => timesAcc(chs, acc + (ch -> (acc(ch) + 1)))
    }
    timesAcc(chars, Map.empty[Char, Int] withDefaultValue(0)).toList
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
    case Nil => Nil
    case first::Nil => encoderHelper(tree)(first)
    case first::second => encoderHelper(tree)(first) ::: encode(tree)(second)
  }

  def encoderHelper(tree: CodeTree)(ch: Char): List[Bit] = tree match {
    case Leaf(c, _) => Nil
    case Fork(l, r, chs, _) => if (chars(l).contains(ch)) 0 :: encoderHelper(l)(ch) else 1  :: encoderHelper(r)(ch)
  }
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    case Leaf(_, _) => Nil
    case Fork(l, r, _, _) => convertAcc(l, List(0)) ::: convertAcc(r, List(1))
  }
