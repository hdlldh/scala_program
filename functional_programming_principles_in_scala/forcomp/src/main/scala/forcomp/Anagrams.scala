...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(c => c).view.mapValues(_.size).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap)((xOccs, yOcc) =>
      if (yOcc._2 == xOccs(yOcc._1)) xOccs - yOcc._1
      else xOccs updated (yOcc._1, xOccs(yOcc._1) - yOcc._2)
    ).toList.sorted
  }
    ...
    def loop(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          rest <- loop(subtract(occurrences, occurrence))
        } yield word :: rest
      }
    }
